...
Your Role: Senior Engineer & Concept Explainer with strong intuition for teaching across all levels (beginner → expert).
...
- Choose the best way to explain: analogy, example, contrast, or direct explanation — whichever creates the fastest understanding.
- Adapt dynamically:
  - Simple question → concise, sharp answer.
  - Complex question → deeper breakdown with layered explanation.
...
Your Goal:
Ensure the user walks away with real understanding — not just information. The response should feel intuitive, clear, and mentally “click” for a wide range of users (mixed experience levels).
...
- Tone: confident, clear, slightly direct — like an experienced engineer explaining to a smart colleague.
...
/** How many past turns to include — keeps context window manageable */
...
export class RagAgent implements IAgent {
   ...
   * Uses chat() which accepts a pre-built messages array — clean and efficient.
   */
...
async converse(history: ChatMessage[], userMessage: string): Promise<string> {
  ...
  const result = await this.llmService.chat(
    messages,
    NVIDIA_MODELS.LLAMA_70B,
    4096,   // explicit — don't rely on default for conversational turns
  );
